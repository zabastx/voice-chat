...
| Notifications | In-app (unread badges, tab-title counter, sounds) + **opt-in Telegram bridge** for offline mentions/DMs with reply-to-send (v0.14.0, [ADR 0006](adr/0006-telegram-notifications.md))    |
...
| Telegram notifications (offline mentions/DMs + reply-to-send)     | ✅ built    | v0.14.0, [ADR 0006](adr/0006-telegram-notifications.md); deep-link `/start` linking, per-notification mapping for replies, auto-unlink on 403, 7-day sweep; `telegram_*` columns are secret (never in `memberDto`). Prod host filters Telegram, so delivery goes through a standalone stateless **`telegram-relay/`** service (webhook + `sendMessage` proxy) — the app talks only to the relay via `NUXT_TELEGRAM_RELAY_URL`, updates return via `POST /api/telegram/ingest` (see ADR "Update: relay transport"). `server/utils/telegram.ts`, `server/api/telegram/ingest.post.ts`, `server/api/me/telegram/*`; typecheck/lint/fmt green — **needs the relay deployed to verify end-to-end** |
...
- **Telegram notifications end-to-end (v0.14.0)** — with a real bot token + public webhook tunnel:
  deep-link `/start` linking binds the chat; an offline `@mention` and an offline DM each deliver
  exactly one notification (online recipient gets none); reply-to-send lands in the right channel
  and the right DM authored as the linked member; non-reply/media/expired-mapping hints; 403
  auto-unlink; and no `telegram_*` field leaks in a `member.updated` frame or `GET /api/members`
