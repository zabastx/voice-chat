...
| Telegram notifications (offline mentions/DMs + reply-to-send)     | ✅ built    | v0.14.0, [ADR 0006](adr/0006-telegram-notifications.md); webhook transport, deep-link `/start` linking, per-notification mapping for replies, auto-unlink on 403, 7-day sweep; `telegram_*` columns are secret (never in `memberDto`); `server/utils/telegram.ts`, `server/api/telegram/webhook.post.ts`, `server/api/me/telegram/*`; typecheck/lint/fmt green — **needs a bot token + public tunnel to verify end-to-end** |
