...
Repo: `zabastx/voice-chat` (infer from `git remote -v` — `gh` does this automatically when run inside a clone).
...
**PRs as a request surface: no.** External PRs are not pulled into the triage queue; only issues are. Collaborators' in-flight PRs are left alone.

If this is later flipped to `yes`, PRs would run through the same labels and states as issues, using the `gh pr` equivalents:
